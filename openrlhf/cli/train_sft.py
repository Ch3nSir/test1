#!/usr/bin/env python3
"""
UniRAG Training Script

This script handles training of the UniRAG model for both stage1 and stage2 training.
"""
...
from openrlhf.models.modeling_unirag import UniRAGConfig, UniRAG
from openrlhf.datasets.sft_dataset import make_collate_fn


def create_unirag_config(args: argparse.Namespace) -> UniRAGConfig:
    """Create UniRAG configuration from command line arguments."""
    return UniRAGConfig(
...
def setup_model(args: argparse.Namespace) -> UniRAG:
    """Setup UniRAG model from arguments."""
    cfg = create_unirag_config(args)
    
    if args.pretrain_checkpoint is not None:
        print(f"Loading model from checkpoint: {args.pretrain_checkpoint}")
        model = UniRAG.from_pretrained(
...
def setup_datasets(args: argparse.Namespace, tokenizer, strategy, model: UniRAG):
...
def setup_training_components(args: argparse.Namespace, model: UniRAG, train_dataset, strategy):
...
def load_checkpoint_if_exists(args: argparse.Namespace, strategy, model: UniRAG) -> int:
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="UniRAG Training Script")
    ...
    model_group.add_argument("--pretrain_checkpoint", type=str, default=None, 
                           help="UniRAG checkpoint to continue training from")
    ...
    # UniRAG specific arguments
    unirag_group = parser.add_argument_group("UniRAG Configuration")
    unirag_group.add_argument("--doc_max_length", type=int, default=256, help="Max document length")
    unirag_group.add_argument("--compress_rate", type=int, default=32, help="Document compression rate")
    unirag_group.add_argument("--qa_loss", action="store_true", default=True, 
                            help="Use QA loss for joint training")
    unirag_group.add_argument("--stage2_mips", action="store_true", default=False, 
                            help="Use MIPS for stage2 retrieval")
    unirag_group.add_argument("--stage2_retrieval_top_n", type=int, default=1, 
                            help="Top-n documents for stage2 retrieval")
    unirag_group.add_argument("--mse_loss", action="store_true", default=False, 
                            help="Add MSE loss during compression training")
    unirag_group.add_argument("--do_eval_gen", action="store_true", default=False, 
    ...
    logging_group.add_argument("--wandb_project", type=str, default="UniRAG", help="Wandb project")
    logging_group.add_argument("--wandb_run_name", type=str, 
                             default="unirag_%s" % datetime.now().strftime("%m%dT%H:%M"),
                             help="Wandb run name")
...
def main():
    ...
    # Print configuration
    print("=" * 60)
    print("UniRAG Training Configuration")
    print("=" * 60)
